...
import { db, auth, collection, addDoc, query, onSnapshot, doc, updateDoc, deleteDoc, onAuthStateChanged } from '../firebase';
...
const CreateScreen = ({ navigation }) => {
  ...
  const [anonymousEarnings, setAnonymousEarnings] = useState(0);
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      ...
      if (currentUser) {
        q = query(collection(db, 'posts'), where('userId', '==', currentUser.uid));
      }
      const unsubscribePosts = onSnapshot(q, (snapshot) => {
        ...
        if (!currentUser) {
          const earnings = loadedPosts.reduce((sum, post) => {
            const responsesCount = responses[post.id]?.length || 0;
            return sum + (responsesCount * 0.10);
          }, 0);
          setAnonymousEarnings(earnings);
        ...
        posts.forEach(post => {
          if (post.active && responseData[post.id]?.length >= post.votes) {
            updateDoc(doc(db, 'posts', post.id), { active: false })
              .catch(error => console.error('Error deactivating post:', error));
          }
        });
      });
  ...
  const handlePreview = () => {
    ...
    if (!user && hasCreatedFirstPost && anonymousEarnings >= EARNINGS_LIMIT) {
  ...
  const handleErasePost = async (postId) => {
    try {
      ...
      console.error('Error erasing post:', error);
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.header, { backgroundColor: headerBackgroundColor, borderBottomWidth: 1, borderBottomColor: theme.isDark ? '#424242' : '#BBDEFB' }]}>
        ...
        <View style={styles.headerRight}>
          {!user && <Text style={[styles.earningsText, { color: theme.text }]}>Earnings: ${anonymousEarnings.toFixed(2)}</Text>}
          {user && (
...
const styles = StyleSheet.create({
  ...
  earningsText: { fontSize: 16, marginRight: 10 },
